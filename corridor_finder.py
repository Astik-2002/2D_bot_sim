...
from kdTree_c import KDNode, KDTree
import random
import numpy as np
import time
...
class RRT_start:
    ...
    def tracePath(self):
        ...
        while ptr is not None:
            print("ptr coordinates: ", ptr.coordinates)
...
def main():
    # Load point cloud
    file_path = "/home/astik/pcd_outputs/2024-09-02_20-08-04/pcd_0000.csv"  # Replace with your file path
...
def plot_final_path(rrt, path, point_cloud):
    ...
    x_range = [min(point_cloud[:, 0]) - 16, max(point_cloud[:, 0])]
    y_range = [min(point_cloud[:, 1]) , max(point_cloud[:, 1])]
